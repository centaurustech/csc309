...
-Added a section on the rationale for the choice of various web technologies.
...
On every page there should be a header with the above mentioned buttons, each of which takes the user to their respective page when clicked on.

There is also a footer at the bottom of each page detailing the name of the team that designed the site.
...
Similarly, the search bar at the page center can be used by any user, whether logged in or not, and it brings the user to the browse page and displays the result of the search.
...
Failing to fill in any of the fields should result in a prompt to the user, while providing an invalid email will redirect the user to a page informing them of this invalidity, with a link that provides them the option of going back and trying again.
Moreover, if the user provides suspicious input, they should be denied the capacity to send the message, and informed that their input was problematic. The link on the confirmation pahe should provide them with the chance to re-submit their input.
Sucessfully filling out the form should lead to a confirmation page that provides a way to get back to the home page.
...
Here the user should be able to update their name, profile picture, biography, and account password information.
...
-Like/Dislike function (if the profile being viewed is that of a friend)
...
The profile should contain all of the basic publically available information for a given user. The name and biography (detailing all the history, as well as relevant skills and experience of the user), should be alterable via the edit button.
The date joined and reputation score are not alterable by the user, but determined automatically. The former is determined automatically upon registration, while the latter is determined by the consideration of other user's of the website.
Individuals become friends with anyone who is already associated with a particular project, and anyone who makes or contributes to a project is said to be associated with said project.
The profile should include a list of community members, which is a master list of all of the friends the user has in every project based community.
The reputation score will be an integer value computed by summing the total number of likes and subtracting the total number of dislikes applied to any given profile.
Only user's who are members of the same community can vote on each other’s profiles and they can do so only once.
...
The user should be able to filter out the projects according to the pre-specified categories provided.
...
This page should allow the user to create a new project by specifying the project name, category, funding goal, and a description. Blank fields should prompt the user to fill them in, and the funding goal must be positive.
...
Other users of the site should be able to rate the project by liking/disliking it, as well as leave testimonials concerning the project. The funding page should be accessible from this point, and all of the information pertaining to the funding goal, such as how much they currently have, and how much is still needed to reach their goal should be present.
...
Users should be able to enter their payment information details as well as the amount of money they want to donate to the given project. All of the fields should be required, and the user should be prompted to fill in any which are left blank. Moreover, non-positive monetary values should not be allowed, and any attempt to use such values should result in the user being prompted to enter funds within a positive range.
...
Finally, administrators will also have the capacity to edit and/or delete any projects irrespective of whether they created it or not.

\textbf{Omissions}

Payment verifier: 

Originally we had intended to integrate some sort of third party payment verification in the back end of the funding page. However, as progress developed, we realized that a live payment verifier was not actually part of the project specification as provided in the hand out, and so we decided to simply implement the front end of the funding page without any actual verification.

The credit card page still allows for live data to be added to the website projects. It is, however, not necessary that the user provide a valid credit card, and there is no charging of any such credit card. Instead, the data provided on the input form is submitted directly to the database to simulate the necessity of managing credit card transactions.
...
users:
This relation will be used to keep track of the authentication credentials for each user. It will also store the user profile data that is dynamically applied to a static profile page template when the user is viewing a particular profile. This relation will also store information used by the project info page to specify details about a projects creator. Like the profile page, this data will by dynamically accessed based on the project and then applied to a static project template page.

transactions:
This relation will store the donations that individual users make via the project info page. This data will be used by the profile page code to extract the contributions of the user whose profile is currently being viewed.

projects:
This relation will store all of the information that is to be dynamically loaded into the static project page template of individual projects. Moreover, it will also be utilized by the information displays that are the result of the search functionality provided by the category filter and project search bar. Finally, this information will interact directly with the profile page, in that the profile page code will use this relation to determine which projects to display as having been created by the user of the current profile being viewed.
